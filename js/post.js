...
const renderComments = (comments) => {
  ...
  if (comments.length > 0) {
    if (!commentsCountLoaded) {
      if (comments.length < COMMENTS_COUNT_START){
        commentsBlock.appendChild(createCommentsFragment(comments.slice(0, comments.length)));
        commentsCountLoaded = comments.length;
      }
      else{
      ...
      if (commentsCountLoaded + COMMENTS_COUNT_LOAD >= comments.length){
        commentsBlock.appendChild(createCommentsFragment(comments.slice(commentsCountLoaded, comments.length)));
        commentsCountLoaded = comments.length;
      }
      else{
